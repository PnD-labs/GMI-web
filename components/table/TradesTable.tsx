...
export const columns: ColumnDef<ITradesData>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-slate-500 text-sm font-bold">{row.getValue("suiAmount")}</div>
    ),
  },
  {
    accessorKey: "tokenMarketCap",
    header: () => (
      <div className="flex justify-start items-center gap-[6px] text-slate-400 text-sm font-bold">
        $ZCAT
      </div>
    ...
    cell: ({ row }) => (
      <div className="text-slate-500 text-sm font-bold">{row.getValue("tokenMarketCap")}</div>
    ),
  },
  {
...
export const TradesTable = ({ data }: { data: ITradesData[] }) => {
